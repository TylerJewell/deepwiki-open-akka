...
/**
 * SPEC-001 R7. Ported verbatim from {@code api/services/wiki/prompts.py}. R7 also carries
 * this port's fix for question-log rows 4/6 (SPEC-001 §4.2): the file tree is joined with
 * newlines here, one path per line, rather than interpolated as a raw list.
 */
public final class WikiPromptBuilder {
  ...
  private WikiPromptBuilder() {}

  public static String languageName(String language) {
    return LANGUAGE_NAMES.getOrDefault(language, "English");
  }

  public static String buildPagePrompt(String title, String fileLinks, String language) {
    return """
        ...
        Immediately after the `<details>` block, the main title of the page should be a H1 Markdown heading: `# %s`.

        Based ONLY on the content of the `[RELEVANT_SOURCE_FILES]`:

        1.  **Introduction:** Start with a concise introduction (1-2 paragraphs) explaining the purpose, scope, and high-level overview of "%s" within the context of the overall project. If relevant, and if information is available in the provided files, link to other potential wiki pages using the format `[Link Text](#page-anchor-or-id)`.

        2.  **Detailed Sections:** Break down "%s" into logical sections using H2 (`##`) and H3 (`###`) Markdown headings. For each section:
        ...
        9.  **Conclusion/Summary:** End with a brief summary paragraph if appropriate for "%s", reiterating the key aspects covered and their significance within the project.

        IMPORTANT: Generate the content in %s language.
        ...
        """
        .formatted(fileLinks, title, title, title, title, languageName(language));
  }

  private static final String COMPREHENSIVE_STRUCTURE =
      """
  ...
  private static final String CONCISE_STRUCTURE =
      """
  ...
  public static String buildStructurePrompt(
      String owner, String repo, List<String> fileTree, String readme, boolean comprehensive, String language) {
    String structureFormat = comprehensive ? COMPREHENSIVE_STRUCTURE : CONCISE_STRUCTURE;
    String pageCount = comprehensive ? "8-12" : "4-6";
    String kind = comprehensive ? "comprehensive" : "concise";
    // R7: joined one-path-per-line, fixing question-log rows 4/6 (SPEC-001 §4.2).
    String fileTreeText = String.join("\n", fileTree);
    return """
        Analyze this GitHub repository %s/%s and create a wiki structure for it.
        ...
        IMPORTANT: The wiki content will be generated in %s language.
        ...
        - State machines
        - Class hierarchies
        %sIMPORTANT FORMATTING INSTRUCTIONS:
        ...
        IMPORTANT:
        1. Create %s pages that would make a %s wiki for this repository
        2. Each page should focus on a specific aspect of the codebase (e.g., architecture, key features, setup)
        3. The relevant_files should be actual files from the repository that would be used to generate that page
        4. Return ONLY valid XML with the structure specified above, with no markdown code block delimiters"""
        .formatted(owner, repo, fileTreeText, readme, languageName(language), structureFormat, pageCount, kind);
  }
}
